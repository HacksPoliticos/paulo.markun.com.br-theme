...
1)		
	...
	----------------------------------------------------------------- */
	function jw_pagination($pages = '', $range = 4){  
		...
		if(1 != $pages){
			...
			<div id="page-nav" class="numbered-pagination">
				<ul>';
				if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<li><a href='".get_pagenum_link(1)."'><span>&laquo;</span></a></li>";
				if($paged > 1 && $showitems < $pages) echo "<li><a href='".get_pagenum_link($paged - 1)."'><span>&lsaquo;</span></a></li>";

				for ($i=1; $i <= $pages; $i++){
					if (1 != $pages &&(!($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems)){
					echo ($paged == $i)? "<li class='current'><a href='".get_pagenum_link($i)."' class='inactive' ><span>".$i."</span></a></li>":"<li><a href='".get_pagenum_link($i)."' class='inactive' ><span>".$i."</span></a></li>";
					}
				}

				if ($paged < $pages && $showitems < $pages) echo "<li><a href='".get_pagenum_link($paged + 1)."'><span>&rsaquo;</span></a></li>";  
				if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<li><a href='".get_pagenum_link($pages)."'><span>&raquo;</span></a></li>";
			echo '
				</ul>
			</div>';
		}
...
7)		
	...
	function jw_breadcrumbs(){
		...
		$homepage_breadcrumbs = false;
		
		$jw_homepage_breadcrumbs = get_option('jw_homepage_breadcrumbs');
		
		if(isset($jw_homepage_breadcrumbs) && $jw_homepage_breadcrumbs == 'on'){
			$homepage_breadcrumbs = true;
		}
		
		/* Show on pages that aren't the main page */
		if ($homepage_breadcrumbs == true || (!is_front_page() && !is_home())) { 
			...
			<?php
				...
				if (is_category() || is_single() || is_author()) {
					...
					if(is_category()){
						if(get_post_type() == 'post'){
							...
							echo '<li class="sep">'.$delimiter.'</li><li>'.get_category_parents($ID, TRUE, FALSE).'</li>';
						}
					}else if(is_author()){
				...
				if((is_single() || is_page()) && (!is_front_page() && !is_home())) { echo '<li class="sep">'.$delimiter.'</li><li><span>'.get_the_title().'</span></li>'; }

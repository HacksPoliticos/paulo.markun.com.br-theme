...
class JW_Slider_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget.'<div class="loopedSlider">';
		
		echo $before_title.$title.'<a href="#" class="previous"></a><a href="#" class="next"></a>'.$after_title;

		$content = '[slider_posts order="'.$order.'" post_type='.$post_type.' amount='.$amount.' tag='.$tag.' '.$dontshow.']';
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'amount' => 5, 'post_type' => 'post', 'tag' => '', 'show_thumbnail' => 'yes', 'show_title' => 'yes', 'show_excerpt' => 'yes', 'order' => 'regular');
		...
		<div class="jw-widget-half first">
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'order' ); ?>"><strong>-<?php _e('Order:', $domain); ?>-</strong></label><br />
				<small>Do you want to show the latest or random posts.</small>
			</p>
			<p>
				<select id="<?php echo $this->get_field_id( 'order' ); ?>" name="<?php echo $this->get_field_name( 'order' ); ?>" class="widefat" style="width:100%;">
					<option value="regular" <?php if ( 'regular' == $instance['order'] ) echo 'selected="selected"'; ?>>Latest</option>
					<option value="random" <?php if ( 'random' == $instance['order'] ) echo 'selected="selected"'; ?>>Random</option>
				</select>
			</p>

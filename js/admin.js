	...
	jQuery(document).ready(function($){
		...
		function generate_composer_code(composer){
			...
			composer_content.find('li').each(function(){
				...
				/* Set the attributes */
				widget_atts = ' width="' + widget_width + '"' + last_attr;
				...
				if(widget_name == 'separator'){ 
					...
					widget_atts = widget_atts + testimonial_type + testimonial_amount + testimonial_columns;
					...
					widget_atts = widget_atts + portfolio_type + portfolio_amount + portfolio_show_thumbnail + portfolio_show_title + portfolio_show_excerpt + portfolio_show_meta + portfolio_category;
					...
					var blog_category = $(this).find('.blog_posts-category-value').val();
					...
					if(blog_category != ''){ blog_category = ' category="' + blog_category + '"'; }
					
					widget_atts = widget_atts + blog_type + blog_amount + blog_show_thumbnail + blog_show_title + blog_show_excerpt + blog_category;
		...
		$('.metabox-composer-widgets ul li a').live('click', function(e){
			...
			widget.find('a').hide().siblings('.metabox-composer-widget-load').show();
			...
			$.get(path_to_composer + '?widget_name=' + widget_name, function(data) {
				...
				widget.find('.metabox-composer-widget-load').hide().siblings('a').show();
		...
		$('.composer-widget-width-more').live('click', function(e){
			...
			if(widget_size == 'one-fourth'){
				...
				widget.removeClass('two-third').addClass('full-width');
		...
		$('.composer-widget-width-less').live('click', function(e){
			...
			if(widget_size == 'one-fourth'){
				...
				widget.removeClass('full-width').addClass('two-third');
				widget_size_input.val('two-third');
				widget_size_info.html('2/3');
			}
		...
		$('.composer-widget-edit').live('click', function(e){
			...
			$.get(path_to_composer + '?widget_name=' + widget_name + '_edit', function(data) {
				...
				if(widget_name == 'service'){
					...
					var blog_category = widget.find('.blog_posts-category-value').val();					
					...
					composer_edit.find('.blog_posts-category-value-new').val(blog_category);
		...
		$('.composer-edit-save').live('click', function(e){
			...
			if(composer_edit.find('.service-edit').length){
				...
				var blog_posts_category = composer_edit.find('.blog_posts-category-value-new').val();
				...
				widget_editing.find('.blog_posts-category-value').val(blog_posts_category);
